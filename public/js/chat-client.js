...
(function () {
  ...
  function appendMessage(data) {
    ...
    if (isSystem) {
      div.innerHTML = `
        <div class="flex items-center gap-3 py-2 px-3 rounded-xl bg-purple-500/8 border border-purple-400/15 text-[12px] text-purple-300 italic">
          📢 ${esc(data.message)}
        </div>`;
    } else if (isNarrator) {
      div.innerHTML = `
        <div class="flex gap-3 px-3 py-3 border-b border-indigo-400/[0.15] bg-indigo-500/[0.04]">
      ...
      div.innerHTML = `
        <div class="flex gap-3 px-3 py-3 border-b border-purple-400/[0.08] hover:bg-white/[0.02] transition-colors">
          <div class="w-10 h-10 rounded-full flex items-center justify-center text-lg shrink-0 mt-0.5 relative" style="border:2px solid ${r.color};background:${r.bg};">
            ${avatarHtml}
          </div>
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 mb-1 flex-wrap pr-1">
              <span class="text-sm font-bold text-slate-100">${esc(data.username)}</span>
              <span class="text-[10px] font-bold px-2 py-0.5 rounded-md border" style="color:${r.color};background:${r.bg};border-color:${r.color}44;">${esc(data.role)}</span>
              ${pts}
              <span class="ml-auto text-[11px] font-mono text-slate-500 tabular-nums">${now()}</span>
            </div>
    ...
    users.forEach(u => {
      ...
      li.innerHTML = `
        ...
        <div class="flex-1 min-w-0">
          ...
          <span class="text-[9px] font-bold px-2 py-0.5 rounded-md inline-block mt-0.5 border" style="color:${r.color};background:${r.bg};border-color:${r.color}44;">${esc(u.role)}</span>
        </div>
  ...
  function renderScoreboard(users) {
    ...
    users.forEach((u, i) => {
      ...
      li.innerHTML = `
        ...
        <span class="text-[9px] font-bold px-1.5 py-0.5 rounded" style="background:${r.color};color:#fff;">${esc(u.role)}</span>
  ...
  socket.on('error_msg', msg => {
    ...
    d.className = 'msg-anim';
    d.innerHTML = `<div class="px-4 py-2 rounded-xl bg-red-500/10 border border-red-400/20 text-[12px] text-red-300">⚠️ ${esc(msg)}</div>`;
